...
soundfile = function () {
      ...
      request.onload = function () {
        if (request.status == 200) {
          ...
          function (buff) {
            ...
            if (buff) {
              console.log('decoded');
              self.bufferSourceNode = self._initSourceNode();
              // garbage collect counterNode and create a new one
              if (self._counterNode)
                self._counterNode = undefined;
              self._counterNode = self._initCounterNode();
              self.bufferSourceNode.connect(self.output);
            }
  ...
  /**
    * creates a new bufferSourceNode and counter
    * to play from a paused time. This will help
    * playing multiple (heavy) track with no latency
    * in diferent browsers.. Chate_89
  **/
  p5.SoundFile.prototype.resetNodes = function() {
...
src_app = function () {
  ...
  var p5SOUND = sndcore;
  return p5SOUND;
}(sndcore, master, helpers, errorHandler, panner, soundfile, amplitude, fft, filter);
}));
