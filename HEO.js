...
function preload() {
  ...
  metadata = loadStrings('data/'+ songName+ '/metadata.json');
}

function setup() {
  numShapes = floor(split(metadata[3],":")[1]);
 ...
 //Check if browser is IE
  ...
  for (var i = 0; i < numShapes; i++) {
    versions[i] = split(metadata[7+(i*6)],":")[1];
  }
  ...
  // load sounds by number of shapes
  if (numShapes === 10) {
    track[9] = loadSound("data/"+ songName+ "/Module0" + (9) + "/00_" + raTr[9] + '.ogg', loaded, null, progress9);
    track[8] = loadSound("data/"+ songName+ "/Module0" + (8) + "/00_" + raTr[8] + '.ogg', loaded, null, progress8);
    track[7] = loadSound("data/"+ songName+ "/Module0" + (7) + "/00_" + raTr[7] + '.ogg', loaded, null, progress6);
    track[6] = loadSound("data/"+ songName+ "/Module0" + (6) + "/00_" + raTr[6] + '.ogg', loaded, null, progress6);
    track[5] = loadSound("data/"+ songName+ "/Module0" + (5) + "/00_" + raTr[5] + '.ogg', loaded, null, progress5);
    track[4] = loadSound("data/"+ songName+ "/Module0" + (4) + "/00_" + raTr[4] + '.ogg', loaded, null, progress4);
    track[3] = loadSound("data/"+ songName+ "/Module0" + (3) + "/00_" + raTr[3] + '.ogg', loaded, null, progress3);
    track[2] = loadSound("data/"+ songName+ "/Module0" + (2) + "/00_" + raTr[2] + '.ogg', loaded, null, progress2);
    track[1] = loadSound("data/"+ songName+ "/Module0" + (1) + "/00_" + raTr[1] + '.ogg', loaded, null, progress1);
    track[0] = loadSound("data/"+ songName+ "/Module0" + (0) + "/00_" + raTr[0] + '.ogg', loaded, null, progress0);
  } else if (numShapes === 9) {
    track[8] = loadSound("data/"+ songName+ "/Module0" + (8) + "/00_" + raTr[8] + '.ogg', loaded, null, progress8);
    track[7] = loadSound("data/"+ songName+ "/Module0" + (7) + "/00_" + raTr[7] + '.ogg', loaded, null, progress6);
    track[6] = loadSound("data/"+ songName+ "/Module0" + (6) + "/00_" + raTr[6] + '.ogg', loaded, null, progress6);
...
function panels() {
  ...
  rect(0, 0, lside, windowHeight);
    ...
    fill(0, 100,200);
    textAlign(LEFT);
    text(infoData[lenguage].version+":", 10, topy-50);
    for (var i = 0; i < raTr.length; i++) {
      text(raTr[i], 10+(i*7), topy-35);
    }
  ...
  text(infoData[lenguage].album, lside-190, windowHeight/2);
  text(infoData[lenguage].artist, lside-190, windowHeight/2+45);
  fill(colours[colsel].inforstr, colours[colsel].infogstr, colours[colsel].infobstr);
  textSize(14);
  text(split(metadata[0],":")[1], lside-190, windowHeight/2-30);
  text(split(metadata[1],":")[1], lside-190, windowHeight/2+15);
  for (var i = 0; i < split(metadata[2],":")[1]; i++) {
    text(split(metadata[2],":")[2+i], lside-190, windowHeight/2+60+(15*i));
  }
  // text(split(metadata[2],":")[2], lside-190, windowHeight/2+75);
